            ...
            try:
                    ...
                    try:
                        if self.fields.keys():
                            if val.tag in self.fields[self._actual_field].keys():
                                self._nr += 1
                                self.fields[self._actual_field]["{}-{}".format(
                                    val.tag, self._nr)] = val.text
                            else:
                                self.fields[self._actual_field][val.tag] = val.text
                    except Exception as e:
                        print(e)
                ...
                if val.tag == 'Enumeration':
                    if self.fields.keys():
                        if 'Enumerations' in self.fields[self._actual_field].keys():
                            self.fields[self._actual_field]['Enumerations'][val.attrib['key']
                                                                            ] = val.attrib['value']
                            if 'requires' in val.attrib:
                                if 'Requires' not in self.fields[self._actual_field]['Enumerations']:
                                    self.fields[self._actual_field]['Enumerations']['Requires'] = {
                                        }
                                self.fields[self._actual_field]['Enumerations']['Requires'][val.attrib['key']
                                                                                            ] = val.attrib['requires']
                ...
                if val.tag == 'Bit':
                    if self.fields[self._actual_field].keys():
                        ...
                        self.fields[self._actual_field][self._actual_bitfield][bitname]['index'] = val.attrib['index']
                        self.fields[self._actual_field][self._actual_bitfield][bitname]['size'] = val.attrib['size']
                        self._actual_bit = bitname

                if val.tag == 'Enumeration':
                    if self._actual_bitfield is not None:
                        if self._actual_bitfield in self.fields[self._actual_field].keys():
                            if self.fields[self._actual_field][self._actual_bitfield].keys():
                                self.fields[self._actual_field][self._actual_bitfield][self._actual_bit][
                                    'Enumerations'][val.attrib['key']] = val.attrib['value']
                                if 'requires' in val.attrib:
                                    if 'Requires' not in self.fields[self._actual_field][self._actual_bitfield][self._actual_bit]['Enumerations']:
                                        self.fields[self._actual_field][self._actual_bitfield][self._actual_bit]['Enumerations']['Requires'] = {
                                            }
                                    self.fields[self._actual_field][self._actual_bitfield][self._actual_bit][
                                        'Enumerations']['Requires'][val.attrib['key']] = val.attrib['requires']

                if val.tag == 'Enumerations':
                    if self._actual_bitfield is not None:
                        if self._actual_bitfield in self.fields[self._actual_field].keys():
                            if self.fields[self._actual_field][self._actual_bitfield].keys():
                                self.fields[self._actual_field][self._actual_bitfield][self._actual_bit][val.tag] = {
                                    }
                ...
                if val.tag == 'Unit':
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.fields[self._actual_field]['Symbol'] = CHARS_UNITS[quantity]
                        except Exception as e:
                            self.fields[self._actual_field][val.tag] = ''
                    ...
                    if self.fields.keys():
                        if hasattr(val.text, 'strip'):
                            self.fields[self._actual_field][val.tag] = ' '.join(
                                ch.capitalize() for ch in val.text.strip().split('.')[-1].split('_'))
                        else:
                            self.fields[self._actual_field][val.tag] = val.text
            except Exception as e:
                print(traceback.format_exc())
...
def _autobitmask(val, total_size, index, size, keymap):
    ...
    _bitmask = eval('0b{}'.format(
        '0'*(total_size-(index+size))+(size*'1')+'0'*index))
            ...
            if 'Ctype' in char.fields[field]:
                ...
                if 'BitField' in char.fields[field]:
                    ...
                    for bitf in bitfield:
                        ...
                        fields[field][bitf] = _autobitmask(val,
            ...
            if 'BitField' in char.fields[field]:
                ...
                for bitf in bitfield:
                    ...
                    if 'Requires' in key_map:
                        fields[field][bitf] = _autobitmask_req(val,
                                                               total_size=total_size,
                                                               index=index,
                                                               size=size,
        ...
        if 'Ctype' in char.fields[field]:
            ...
            if 'BitField' in char.fields[field]:
                ...
                if 'DecimalExponent' in char.fields[field]:
                    formatted_value *= 10**(char.fields[field]
                                            ['DecimalExponent'])
                if 'BinaryExponent' in char.fields[field]:
                    formatted_value *= 2**(char.fields[field]
                                           ['BinaryExponent'])
        ...
        for field in char.fields:
            if field != 'Flags':
                ...
                if 'Mandatory' in field_req:
                    ...
                    _READ_FIELD = all([req in _REQS.values()
        ...
        if _FIELDS_TO_READ:
            ...
            for field in _FIELDS_TO_READ:
                ...
                if field in char.fields:
                    ...
                    if 'DecimalExponent' in char.fields[field]:
                        formatted_value *= 10**(
                            char.fields[field]['DecimalExponent'])
                    if 'BinaryExponent' in char.fields[field]:
                        formatted_value *= 2**(
                            char.fields[field]['BinaryExponent'])
                    if 'BitField' in char.fields[field]:
                        formatted_value = list(_autoformat(
                            char, formatted_value, field).values())[0]
...
def get_char_value(char, val, rtn_flags=False, debug=False):
    ...
    if len(char.fields) == 1:
        ...
        return _get_multiple_fields(char, val, rtn_flags=rtn_flags,
                                    debug=debug)


def pformat_char_value(data, char='', only_val=False, one_line=False, sep=',',
                       custom=None, symbols=True, prnt=True, rtn=False):
    if not custom:
        if not one_line:
            if char:
                print('{}:'.format(char))
